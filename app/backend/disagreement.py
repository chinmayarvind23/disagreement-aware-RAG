...
class DisagreeHead:
    def __init__(self):
        self.model = None
        self.threshold = 0.45  # abstainance threshold, when p_disagree < threshold, we answer
    ...
    def load(cls, path: str):
        ...
        h = cls()
        h.model = obj["model"]
        h.threshold = obj.get("threshold", 0.45)
        return h

def decision_from_feats(p_disagree: float, feats: dict, tau=0.45, min_overlap=0.3, max_sc=0.5):
